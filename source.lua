...
local prompt = useStudio and require(script.Parent.prompt) or loadWithTimeout('https://raw.githubusercontent.com/AkiraMeowSato/Lib/refs/heads/main/prompt.lua')
...
local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://raw.githubusercontent.com/AkiraMeowSato/Lib/refs/heads/main/icons.lua')
-- Variables
...
local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = SexTalityLib.Theme[Theme]
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(SexTalityLib.Flags) do
...
function SexTalityLib:Notify(data) -- action e.g open messages
	task.spawn(function()
...
function SexTalityLib:CreateWindow(Settings)
	...
	if (Settings.KeySystem) then
		...
		if not Passthrough then
            ...
            -- Set the Text and Colors immediately
            KeyMain.Title.Text = Settings.KeySettings.Title or Settings.Name
            KeyMain.Title.TextColor3 = Color3.fromRGB(255, 31, 51) -- Fatality Red Title
            
            KeyMain.Subtitle.Text = Settings.KeySettings.Subtitle or "Key System"
            KeyMain.Subtitle.TextColor3 = Color3.fromRGB(206, 191, 209)
            
            KeyMain.NoteMessage.Text = Settings.KeySettings.Note or "No instructions"
            KeyMain.NoteMessage.TextColor3 = Color3.fromRGB(206, 191, 209)
            
            -- Set Background Colors
            KeyMain.BackgroundColor3 = Color3.fromRGB(14, 10, 24) -- Dark Fatality Background
            KeyMain.Input.BackgroundColor3 = Color3.fromRGB(13, 13, 29) -- Input Field Background
            KeyMain.Input.UIStroke.Color = Color3.fromRGB(255, 31, 51) -- Red Stroke around Input
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= SexTalityLib.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					SexTalityLib.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
		...
		function Tab:CreateInput(InputSettings)
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					SexTalityLib.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					SexTalityLib.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					SexTalityLib.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						SexTalityLib.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					SexTalityLib.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			SexTalityLib:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			SexTalityLib:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
...
function SexTalityLib:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function SexTalityLib:IsVisible(): boolean
	return not Hidden
end
...
if useStudio then
	...
	--local thoptions = {}
	--for themename, theme in pairs(SexTalityLib.Theme) do
...
task.delay(4, function()
	SexTalityLib.LoadConfiguration()
